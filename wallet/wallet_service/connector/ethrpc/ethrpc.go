...
func (rpc *EthRPC) callReceipt(params ...interface{}) (*TransactionReceipt,map[string]interface{},error) {
	result, err := rpc.Call("eth_getTransactionReceipt", params...)
	if err != nil {
		return nil,nil,err
	}
...
func (rpc *EthRPC) EthGetBalanceV2(address, block string) (big.Int, error) {
	return rpc.EthGetBalance(address,block)
}
...
func (rpc *EthRPC) SetUrl(url string)  {
	rpc.url = url
}

func (rpc *EthRPC) EthGetTransactionReceipt(hash string) (*TransactionReceipt, error) {
	receipt,receipt_map,err := rpc.callReceipt(hash)
	if err != nil {
		return nil, err
	}
	...
	if receipt_map == nil {
		receipt.Status = 0
	} else if receipt_map["status"] == "0x1" {
		receipt.Status = 1
	} else if receipt_map["status"] == "0x0" {
		receipt.Status = -1
	}
